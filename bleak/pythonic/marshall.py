...
class BleakGATTMarshaller(abc.ABC):
    ...
    """
    @classmethod
    async def get_marshaller(klass, client: BleakClient, uuid: str, descr_2904=None) -> Any:
        ...
        if descr_2904:
            ...
            if format in Table_2904_bytes_format:
                m = BleakGATTPackMarshaller(Table_2904_bytes_format[format], exponent, unit)
                # We could add the marshaller to the table here, if we have a uuid
                return m
        warnings.warn(f"BleakGATTMarshaller: no marshaller found for client {client.address} characteristic {uuid}")
